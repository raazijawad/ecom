...
const dashboardSections: { id: DashboardSection; label: string }[] = [
    ...
    { id: 'profile', label: 'Profile / Account Information' },
    { id: 'password', label: 'Change Password' },
    { id: 'address', label: 'Address Book' },
    { id: 'wishlist', label: 'Wishlist' },
    { id: 'cart', label: 'Cart Preview' },
    { id: 'notifications', label: 'Notifications / Alerts' },
    { id: 'coupons', label: 'Coupons / Rewards' },
    { id: 'support', label: 'Support / Help Center' },
];

export default function CustomerDashboard() {
    const { auth, recentOrders } = usePage<SharedProps>().props;
    const [activeSection, setActiveSection] = useState<DashboardSection>('recent-orders');
    ...
    const renderSectionContent = () => {
        switch (activeSection) {
                                        ...
                                        recentOrders.map((order) => (
                                            <tr key={order.id} className={order.id === latestOrder?.id ? 'bg-blue-50/50' : ''}>
                                                ...
                                                <td className="px-4 py-3">
                                                    <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold uppercase tracking-wide text-slate-700">{order.status}</span>
                                                </td>
                ...
                return (
                    <>
                        ...
                        <form className="mt-4 space-y-3" onSubmit={handleProfileSubmit}>
                            ...
                            {profileMessage ? <p className={profileMessage.type === 'success' ? 'text-sm text-emerald-600' : 'text-sm text-rose-600'}>{profileMessage.text}</p> : null}
                        </form>
                    </>
                );
            case 'password':
                return (
                    <>
                        ...
                        <form className="mt-4 space-y-3" onSubmit={handlePasswordSubmit}>
                            ...
                            {passwordMessage ? <p className={passwordMessage.type === 'success' ? 'text-sm text-emerald-600' : 'text-sm text-rose-600'}>{passwordMessage.text}</p> : null}
                        </form>
                    </>
                );
            case 'address':
                return (
                    <>
                            ...
                            {addresses.map((address) => (
                                <article key={address.id} className="rounded-xl border border-slate-200 p-4">
                                    ...
                                    <div className="mt-3 flex gap-2">
                                        <button type="button" onClick={() => startAddressEdit(address)} className="rounded-md border border-slate-300 px-3 py-1 text-xs font-semibold">
                                            Edit Address
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => setAddresses((current) => current.filter((item) => item.id !== address.id))}
                                            className="rounded-md border border-rose-300 px-3 py-1 text-xs font-semibold text-rose-600"
                                        >
                                            Delete Address
                                        </button>
                                    </div>
                        ...
                        <form className="mt-5 grid gap-3 md:grid-cols-2" onSubmit={handleAddressSave}>
                            ...
                            <input className="rounded-lg border border-slate-300 px-3 py-2 text-sm md:col-span-2" placeholder="Address line" value={addressDraft.line1} onChange={(event) => setAddressDraft({ ...addressDraft, line1: event.target.value })} />
                            ...
                            <div className="md:col-span-2 flex gap-2">
                                <button className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">{editingAddressId ? 'Update Address' : 'Add Address'}</button>
                                {editingAddressId ? (
                                    <button type="button" onClick={resetAddressDraft} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700">
                                        Cancel
                                    </button>
                                ) : null}
                            </div>
                        ...
                        <ul className="mt-4 space-y-3">
                            {wishlist.map((item) => (
                                <li key={item.id} className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setWishlist((current) => current.filter((wishlistItem) => wishlistItem.id !== item.id))}
                                        className="rounded-md border border-rose-300 px-3 py-1 text-xs font-semibold text-rose-600"
                                    >
                                        Remove
                                    </button>
                        ...
                        <ul className="mt-4 space-y-3 text-sm">
                            {starterCartItems.map((item) => (
                                <li key={item.id} className="flex items-center justify-between rounded-xl border border-slate-200 p-3">
                                    ...
                                    <p className="text-slate-700">{item.total}</p>
                                </li>
                            ))}
                        </ul>
                        <p className="mt-4 text-sm font-semibold text-slate-900">Estimated total: ${cartTotal}</p>
                    </>
                );
            case 'notifications':
                return (
            ...
            case 'coupons':
                return (
                    <>
                        <h2 className="text-xl font-semibold text-slate-900">Coupons / Rewards</h2>
                        <div className="mt-4 space-y-3 text-sm text-slate-700">
                            <p className="rounded-lg bg-emerald-50 px-3 py-2 text-emerald-700">ACTIVE COUPON: SAVE15 (expires in 3 days)</p>
                            <p>
                                Reward points: <span className="font-semibold">240 points</span>
                            </p>
            ...
            case 'support':
                return (
                    <>
                        <h2 className="text-xl font-semibold text-slate-900">Support / Help Center</h2>
                        <form className="mt-4 space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                <button type="button" className="rounded-md bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white">
                                    Contact Support
                                </button>
                                <button type="button" className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700">
                                    FAQ
                                </button>
                                <button type="button" className="rounded-md border border-blue-300 px-3 py-1.5 text-xs font-semibold text-blue-700">
                                    Open Ticket
                                </button>
                            </div>
                        </form>
    ...
    return (
        <ShopLayout title="Customer Dashboard">
            ...
            <div className="space-y-6 pb-10">
                ...
                <section className="grid gap-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[280px_1fr] md:p-6">
                    <aside className="border-b border-slate-200 pb-4 md:border-b-0 md:border-r md:pb-0 md:pr-4">
                        <p className="mb-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Dashboard Menu</p>
                            ...
                            {dashboardSections.map((section) => (
                                <button
                                    ...
                                    className={`w-full rounded-lg px-3 py-2 text-left text-sm font-medium transition ${
                                        activeSection === section.id ? 'bg-slate-900 text-white' : 'text-slate-700 hover:bg-slate-100'
                                    }`}
                                >
                                    {section.label}
                                </button>
                    ...
                    <div className="min-h-[360px] p-1">{renderSectionContent()}</div>
                </section>
