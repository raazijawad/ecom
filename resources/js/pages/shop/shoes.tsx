...
export default function Shoes({ products, cartSummary }: Props) {
    ...
    useEffect(() => {
        ...
        setActiveCategory((currentCategory) => currentCategory || categorizedProducts[0].id);
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                ...
                if (visibleCategory?.target.id) {
                    setActiveCategory(visibleCategory.target.id);
                }
            },
            {
                            ...
                            <li key={id}>
                                <button
                                    ...
                                    className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                                        activeCategory === id ? 'bg-blue-600 text-white shadow' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                                    }`}
                                >
                                    {category}
                                </button>
